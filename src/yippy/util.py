...
def create_shift_mask_jax(psf, shift_x, shift_y, fill_val=1):
    """Create a mask to identify valid pixels to average.
    ...
    if shift_x > 0:
        # Zero out the left side
        mask = mask.at[:, : int(np.ceil(shift_x))].set(0)
    elif shift_x < 0:
        # Zero out the right side
        mask = mask.at[:, int(np.floor(shift_x)) :].set(0)

    # Handle y-direction shifting
    if shift_y > 0:
        # Zero out the bottom side
        mask = mask.at[: int(np.ceil(shift_y)), :].set(0)
    elif shift_y < 0:
        # Zero out the top side
        mask = mask.at[int(np.floor(shift_y)) :, :].set(0)
